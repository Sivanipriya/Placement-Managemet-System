...
// mysql_select_db("placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur)
	{
		...
		{
			$sql = $connect->query("SELECT * FROM `details`.`slogin` WHERE `USN`='$Username'");
			if($sql->num_rows == 1)
			{
				$row = $sql->fetch_assoc();
				...
				if($cur == $dbpassword)
				{
					if($query =$connect->query("UPDATE `details`.`slogin` SET `PASSWORD` = '$Password' WHERE `slogin`.`USN` = '$Username'"))
					{
